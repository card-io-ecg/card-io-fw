...
use device_descriptor::{Proxy, ReadOnlyRegister, Register, RegisterWidthType};
...
impl<T> RegisterReader<<T as Proxy>::RegisterWidth> for T
where
    T: ReadOnlyRegister,
{
    fn read<E>(
        iface: &mut impl RegisterAccess<<T as Proxy>::RegisterWidth, Error = E>,
    ) -> Result<Self, E> {
        iface.read_register()
    }
}

impl<T: ReadOnlyRegister> AsyncRegisterReader<<T as Proxy>::RegisterWidth> for T {
    async fn read_async<E>(
        iface: &mut impl AsyncRegisterAccess<<T as Proxy>::RegisterWidth, Error = E>,
    ) -> Result<Self, E> {
        iface.read_register_async().await
    }
}

impl<T: Register> RegisterWriter<<T as Proxy>::RegisterWidth> for T {
    fn write<E>(
        self,
        iface: &mut impl RegisterAccess<<T as Proxy>::RegisterWidth, Error = E>,
    ) -> Result<(), E> {
...
impl<T: Register> AsyncRegisterWriter<<T as Proxy>::RegisterWidth> for T {
    async fn write_async<E>(
        self,
        iface: &mut impl AsyncRegisterAccess<<T as Proxy>::RegisterWidth, Error = E>,
    ) -> Result<(), E> {
    ...
    RWT: RegisterWidthType,
{
    ...
    where
        R: ReadOnlyRegister + Proxy<RegisterWidth = RWT>;
    fn write_register<R>(&mut self, reg: R) -> Result<(), Self::Error>
    where
        R: Register + Proxy<RegisterWidth = RWT>;
